...
- **Unified Branding & Navigation Layout Rules**:
  ...
  - **Back Button Alignment & Fixed Padding**: Header padding is set to a constant `0 20px` globally via the `.hdr` stylesheet class. Inline dynamic left padding overrides have been completely removed from all React files so that the Back button and title remain constantly aligned 20px from the left edge of the screen, matching parent index pages.
